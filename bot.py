...
BASE_MIN_CONFIDENCE = 78
NEUTRAL_ALT_MIN_CONFIDENCE = 90
BUILD_ID = "V4.1-ALL8-VERIFIED"
MIN_ADAPTIVE_SAMPLES = 8
...
SKIP_ALTS_WHEN_BTC_NEUTRAL = False

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
)
log = logging.getLogger("v36-adaptive")
...
def detect_fvg(df: pd.DataFrame, side: str) -> bool:
    ...
    for i in range(len(recent) - 3, 1, -1):
        ...
        if side == "BUY" and candle_3["low"] > candle_1["high"]:
            return bool(current["low"] <= candle_3["low"] and current["close"] >= candle_1["high"])

        if side == "SELL" and candle_3["high"] < candle_1["low"]:
            return bool(current["high"] >= candle_3["high"] and current["close"] <= candle_1["low"])

    return False


def detect_order_block(df: pd.DataFrame, side: str) -> bool:
    ...
    for i in range(len(recent) - 2, 1, -1):
        ...
        if side == "BUY":
            if (
                ...
                and (impulse["close"] - impulse["open"]) > 0.8 * atr
            ):
                return bool(current["low"] <= candle["open"] and current["close"] > candle["low"])

        if side == "SELL":
            if (
                candle["close"] > candle["open"]
                and impulse["close"] < impulse["open"]
                and (impulse["open"] - impulse["close"]) > 0.8 * atr
            ):
                return bool(current["high"] >= candle["open"] and current["close"] < candle["high"])
...
def adaptive_adjustment(pattern: str, performance: dict) -> tuple[int, str]:
    stats = performance.get("pattern_stats", {}).get(pattern)
    if not stats or stats.get("trades", 0) < MIN_ADAPTIVE_SAMPLES:
    ...
    if win_rate >= 0.70:
        return 8, f"historical win rate {win_rate:.0%}"
    if win_rate >= 0.60:
        return 4, f"historical win rate {win_rate:.0%}"
    if win_rate <= 0.35:
        return -10, f"historical win rate {win_rate:.0%}"
    if win_rate <= 0.45:
        return -5, f"historical win rate {win_rate:.0%}"
    return 0, f"historical win rate {win_rate:.0%}"


def build_signal(exchange, symbol: str, btc_trend: str, performance: dict) -> Optional[tuple[Signal, pd.DataFrame]]:
    ...
    for side in ("BUY", "SELL"):
        ...
        # In a neutral BTC regime, altcoins may still qualify, but only with
        # an exceptional confidence threshold. Neutral BTC does not add score.
        market_regime_allowed = market_regime_match or btc_trend == "neutral"
        ...
        bos, choch, _ = detect_bos_choch(m15, side)
        sweep = detect_liquidity_sweep(m15, side)
        fvg = detect_fvg(m15, side)
        ...
        pkey = pattern_key(side, bos, choch, sweep, fvg, order_block)
        adjustment, history_note = adaptive_adjustment(pkey, performance)
        adaptive = max(0, min(100, raw + adjustment))
        ...
        if adjustment != 0:
            reasons.append(f"Adaptive score {adjustment:+d}: {history_note}")

        log.info(
            "%s %s | raw=%s adaptive=%s pattern=%s | BTC=%s 4H=%s 1H=%s",
            symbol,
            side,
            raw,
            adaptive,
        ...
        required_confidence = (
            NEUTRAL_ALT_MIN_CONFIDENCE
            if btc_trend == "neutral" and symbol != BTC_SYMBOL
            else BASE_MIN_CONFIDENCE
        )
        if mandatory and adaptive >= required_confidence:
            if btc_trend == "neutral" and symbol != BTC_SYMBOL:
                reasons.append(
                    f"Exceptional altcoin setup during neutral BTC (minimum {required_confidence}%)"
                )
            candidates.append((adaptive, raw, side, pkey, tuple(reasons)))

    if not candidates:
        return None

    candidates.sort(key=lambda item: item[0], reverse=True)
    adaptive, raw, side, pkey, reasons = candidates[0]
...
def update_outcomes(exchange, performance: dict) -> list[str]:
    ...
    for key, trade in list(open_signals.items()):
        try:
            df = fetch_candles(exchange, trade["symbol"], LTF)
            future = df[df["timestamp"] > int(trade["timestamp"])]
            if future.empty:
                continue
...
def performance_summary(performance: dict) -> str:
    ...
    if not closed:
        return "No completed tracked signals yet."

    wins = sum(1 for trade in closed if trade.get("outcome") == "WIN")
    losses = sum(1 for trade in closed if trade.get("outcome") == "LOSS")
    total = wins + losses
    win_rate = wins / total if total else 0

    return (
        f"Tracked results: {total}\n"
        f"Wins: {wins}\n"
        f"Losses: {losses}\n"
        f"TP1 win rate: {win_rate:.1%}"
    )
...
def create_chart(df: pd.DataFrame, signal: Signal) -> str:
    ...
    for i, row in enumerate(chart_df.itertuples()):
        ax.vlines(i, row.low, row.high, linewidth=1)
        bottom = min(row.open, row.close)
        height = max(abs(row.close - row.open), signal.entry * 0.00005)
        ax.add_patch(
            plt.Rectangle(
                (i - 0.3, bottom),
                0.6,
    ...
    ax.set_title(
        f"{signal.symbol} {signal.side} | Adaptive {signal.adaptive_confidence}% | 15m"
    )
...
def main() -> None:
    ...
    if btc_trend == "neutral" and SKIP_ALTS_WHEN_BTC_NEUTRAL:
        scan_symbols = [BTC_SYMBOL] if BTC_SYMBOL in available else []
        log.info("BTC is neutral; altcoin scan skipped")
    else:
        scan_symbols = available

    for symbol in scan_symbols:
        try:
            result = build_signal(exchange, symbol, btc_trend, performance)
            if result is None:
                continue
            ...
            send_telegram_text(format_signal(signal))
    ...
    if btc_trend == "neutral" and not SKIP_ALTS_WHEN_BTC_NEUTRAL:
        summary += (
            f"\n\n🔎 All pairs scanned. Neutral-BTC altcoins require "
            f"{NEUTRAL_ALT_MIN_CONFIDENCE}% confidence."
        )
